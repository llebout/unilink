  ...
  for (line = NULL, n = 0, nline = 0;
    ...
    case 0:
      ...
      if (port > 0 && port < 65535) {
        pi->port = strdup(line);
        if (pi->port == NULL) {
          fprintf(stderr, "read_peerinfo(); port strdup failed\n");
          ret = -4;
        }
      } else {
        fprintf(stderr, "read_peerinfo(); invalid port\n");
        ret = -5;
      }
      break;
    case 1:
      if (strcmp(line, "x25519") == 0) {
        ...
        if (pi->alg_pk == NULL) {
          fprintf(stderr, "read_peerinfo(); pk strdup failed\n");
          ret = -6;
        }
      } else {
        fprintf(stderr, "read_peerinfo(); invalid alg_pk\n");
        ret = -7;
      }
      break;
    case 2:
      ...
      */
      ...
      if (bin == NULL) {
        fprintf(stderr, "read_peerinfo(); bin malloc failed\n");
        ret = -8;
      } else {
        if (sodium_base642bin(
                bin, b64_len / 4 * 3 + 2, line, b64_len, NULL, &bin_len,
                &b64_end, sodium_base64_VARIANT_ORIGINAL_NO_PADDING) == -1) {
          fprintf(stderr, "read_peerinfo(); invalid pk\n");
          free(bin);
          ret = -9;
        } else {
          pi->pk_size = bin_len;
      ...
      */
      ...
      if (bin == NULL) {
        ...
        if (sodium_base642bin(
                bin, b64_len / 4 * 3 + 2, line, b64_len, NULL, &bin_len,
                &b64_end, sodium_base64_VARIANT_ORIGINAL_NO_PADDING) == -1) {
          fprintf(stderr, "read_peerinfo(); invalid sk\n");
          free(bin);
          ret = -11;
        } else {
          pi->sk_size = bin_len;
    ...
    case 4:
      if (strcmp(line, "x25519") == 0) {
        ...
        if (pi->master_alg_pk == NULL) {
          fprintf(stderr, "read_peerinfo(); master_pk strdup failed\n");
          ret = -12;
        }
      } else {
        fprintf(stderr, "read_peerinfo(); invalid master_alg_pk\n");
        ret = -13;
      }
      break;
    case 5:
      ...
      */
      ...
      if (bin == NULL) {
        ...
        if (sodium_base642bin(
                bin, b64_len / 4 * 3 + 2, line, b64_len, NULL, &bin_len,
                &b64_end, sodium_base64_VARIANT_ORIGINAL_NO_PADDING) == -1) {
          fprintf(stderr, "read_peerinfo(); invalid master_pk\n");
          free(bin);
          ret = -15;
        } else {
      ...
      break;
    case 6:
      if (sscanf(line, "%" SCNu32, &pi->master_sequence_num) != 1) {
        fprintf(stderr, "read_peerinfo(); invalid master_sequence_num\n");
        ret = -16;
      }
      break;
    }
...
int write_peerinfo(struct peerinfo *pi) {
  ...
  sodium_bin2base64(b64_pk, b64_pk_maxlen, pi->pk, pi->pk_size,
                    sodium_base64_VARIANT_ORIGINAL_NO_PADDING);

  b64_sk_maxlen = sodium_base64_ENCODED_LEN(
  ...
  sodium_bin2base64(b64_sk, b64_sk_maxlen, pi->sk, pi->sk_size,
                    sodium_base64_VARIANT_ORIGINAL_NO_PADDING);

  b64_master_pk_maxlen = sodium_base64_ENCODED_LEN(
      pi->master_pk_size, sodium_base64_VARIANT_ORIGINAL_NO_PADDING);
  ...
  sodium_bin2base64(b64_master_pk, b64_master_pk_maxlen, pi->master_pk,
                    pi->master_pk_size,
                    sodium_base64_VARIANT_ORIGINAL_NO_PADDING);
  ...
  s = fprintf(fp,
              ...
              "%s\n"           /* master_pk (base64) */
              "%" SCNu32 "\n", /* master_sequence_num */
              pi->port, pi->alg_pk, b64_pk, b64_sk, pi->master_alg_pk,
              b64_master_pk, pi->master_sequence_num);
